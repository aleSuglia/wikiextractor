...
IT_DBPEDIA_ENDPOINT = "http://it.dbpedia.org/sparql"

sparql_client = SPARQLWrapper(IT_DBPEDIA_ENDPOINT, returnFormat=JSON)
global sparql_client


def get_redirect_url(it_resource):
    try:
        normalized_url = quote(it_resource, safe=":/")
        resp = urlopen(normalized_url)
        return unquote(resp.geturl().replace("https", "http"))
    except urllib.error.HTTPError as ex:
        logging.error("Missing URL {}, trying with the english page".format(normalized_url))
        normalized_url = normalized_url.replace("it", "en")
        try:
            resp = urlopen(normalized_url)
            return unquote(resp.geturl().replace("https", "http"))
        except urllib.error.HTTPError as ex:
...
def get_resource_id(it_resource):
    query = """
    select ?en_entity ?it_entity where {
     ?it_entity foaf:isPrimaryTopicOf <%s>.
     OPTIONAL {
        ?it_entity owl:sameAs ?en_entity.
        FILTER(REGEX(str(?en_entity), "^http://dbpedia.org/resource"))
     }
    }
    """ % it_resource
    ...
    if "bindings" in results["results"]:
        ...
        for b in bindings:
            if "en_entity" in b:
                en_entity_uri = b["en_entity"]["value"]
            if "it_entity" in b:
                it_entity_uri = b["it_entity"]["value"]

        if it_entity_uri:
            slash = it_entity_uri.rfind("/")
            if slash > 0:
                entity_label = it_entity_uri[slash+1:]
                return entity_label

        # found match in results
        if en_entity_uri:
            slash = en_entity_uri.rfind("/")
            if slash > 0:
                entity_label = en_entity_uri[slash:]
                return entity_label
...
def makeInternalLink(title, label):
    ...
    if options.keepLinks:
        ...
        if options.keepEntities:
            # take italian label and obtain the english one (universal identifier)
            raw_url = "http://it.wikipedia.org/wiki/{}".format(title.replace(" ", "_")).encode("utf-8")
            wiki_url = get_redirect_url(raw_url)
            if wiki_url:
                logging.info(wiki_url)
                entity_id = get_resource_id(wiki_url)
...
def main():
    ...
    groupP.add_argument("-l", "--links", action="store_true",
                        help="preserve links")
    groupP.add_argument("-e", "--entities", action="store_true",
                        help="preserve entities")
    ...
    options.keepEntities = args.entities
